...
local function closeTrunk()
	if currentInventory?.type == 'trunk' then
		...
		Utils.PlayAnimAdvanced(1000, 'anim@heists@fleeca_bank@scope_out@return_case', 'trevor_action', coords.x, coords.y, coords.z, 0.0, 0.0, GetEntityHeading(playerPed), 2.0, 2.0, 1000, 49, 0.25)
	...
	if canUseItem((data.ammo) and true) then
		...
		elseif currentWeapon then
            ...
            elseif data.clip then
                ...
                for k,v in pairs(weaponComponents) do
                    if string.match(v, 'clip') then
                        local clipToGive = v
                        local ammoToGive = 0
                        local currentAmmo = currentWeapon.metadata.ammo
                        maxAmmo = Items[v].ammocount

                        if currentAmmo ~= maxAmmo then
                            clipToGive = clipToGive .. 'empty'
                            ammoToGive = currentAmmo
                        end

                        --Check if the component is really on the weapon and remove it
                        if Items[v].client then
                            for _, component in pairs(Items[v].client.component) do
                                if HasPedGotWeaponComponent(playerPed, currentWeapon.hash, component) then
                                    local success = lib.callback.await('ox_inventory:updateWeapon', false, 'component', k, nil, nil, clipToGive, ammoToGive)
                                    if success then
                ...
                if cache.vehicle then
                    SetAmmoInClip(playerPed, currentWeapon.hash, maxAmmo)

                    if cache.seat > -1 or IsVehicleStopped(cache.vehicle) then
                        TaskReloadWeapon(playerPed, true)
                    end
                else
                    AddAmmoToPed(playerPed, currentWeapon.hash, maxAmmo)
                    Wait(100)
                    MakePedReload(playerPed)
                ...
                lib.callback.await('ox_inventory:updateWeapon', false, 'loadclip', maxAmmo)
			...
			elseif data.component then
				...
				lib.notify({ id = 'component_invalid', type = 'error', description = locale('component_invalid', label) })
			elseif data.allowArmed ~= false then
				useItem(data)
			end
	...
	client.interval = SetInterval(function()
				...
				if currentInventory and not currentInventory.ignoreSecurityChecks then
					if currentInventory.type == 'otherplayer' then
						...
						if not id or #(playerCoords - pedCoords) > 1.8 or not (client.hasGroup(shared.police) or canOpenTarget(ped)) then
							...
							TaskTurnPedToFaceCoord(playerPed, pedCoords.x, pedCoords.y, pedCoords.z, 500)
						end
		...
		elseif client.weaponmismatch and not client.ignoreweapons[weaponHash] then
			...
			if weaponType ~= 0 and weaponType ~= `GROUP_UNARMED` then
                TriggerServerEvent('MyCity_Core:CheatEventsNoWeapon:Logs', weaponHash, true)
...
RegisterNUICallback('removeComponent', function(data, cb)
	...
	if not currentWeapon then
		return lib.notify({ id = 'weapon_hand_wrong', type = 'error', description = locale('vous devez avoir l\'arme en main') })
	end
    ...
    if Items[data.component].client then
