...
function playUnholsterAnim()
    ...
    if holsterStyle == 0 then
        Utils.PlayAnimAdvanced(600, "reaction@intimidation@1h", "intro", coords.x, coords.y, coords.z, 0, 0, GetEntityHeading(playerPed), 8.0, 3.0, -1, 50, 0.325)
...
function playHolsterAnim()
    local playerPed = PlayerPedId()
    local coords = GetEntityCoords(cache.ped, true)
    if holsterStyle == 0 then
        -- Par Défault (Holster Derrière)
        Utils.PlayAnimAdvanced(1000, 'reaction@intimidation@1h', 'outro', coords.x, coords.y, coords.z, 0, 0, GetEntityHeading(cache.ped), 8.0, 3.0, -1, 50, 0.125)
    elseif holsterStyle == 1 then
        -- Hoster Droite
    ...
    elseif holsterStyle == 4 then
        --Holster Jambe
        Utils.PlayAnimAdvanced(500, "reaction@male_stand@big_variations@d", "react_big_variations_m", coords.x, coords.y, coords.z, 0, 0, GetEntityHeading(cache.ped), 8.0, 3.0, -1, 50, 0.125)
    end
end
...
function Weapon.Equip(item, data, noWeaponAnim)
	...
	if item.metadata.components then
		for i = 1, #item.metadata.components do
			local componentsClient = Items[item.metadata.components[i]].client
            if componentsClient and componentsClient.component then
				for k,v in pairs(componentsClient.component) do
					if DoesWeaponTakeWeaponComponent(data.hash, v) then
						if not HasPedGotWeaponComponent(playerPed, data.hash, v) then
							GiveWeaponComponentToPed(playerPed, data.hash, v)
						end
					end
				end
            end
