...
local function openInventory(source, invType, data, ignoreSecurityChecks)
	...
	if data then
		...
		elseif invType == 'policeevidence' then
			if server.hasGroup(left, "sheriff") then
				right = Inventory(('sheriffevidence-%s'):format(data))
			elseif server.hasGroup(left, shared.police) then
...
AddEventHandler('onServerResourceStart', function(resourceName)
	if resourceName == 'ox_inventory' or resourceName == GetCurrentResourceName then
		local jobs = MySQL.query.await('SELECT name FROM jobs', {})
        for k,v in pairs(jobs) do
            exports.ox_inventory:RegisterStash(v.name .. '_storage', "Coffre", 500, 10000000, false, v.name)
        end

        exports.ox_inventory:RegisterStash('police_storage2', "Coffre", 500, 10000000, false, 'police')

        local factions = MySQL.query.await('SELECT name FROM factions', {})
        for k,v in pairs(factions) do
            exports.ox_inventory:RegisterStash(v.name .. '_armory', "Armurerie", 500, 10000000, false, v.name)
            exports.ox_inventory:RegisterStash(v.name .. '_armoryboss', "Armurerie Haut Gradés", 500, 10000000, false, v.name)
            exports.ox_inventory:RegisterStash(v.name .. '_armorycayo', "Armurerie Cayo", 500, 10000000, false, v.name)
        end

        local properties = MySQL.query.await('SELECT id FROM properties', {})
        for k,v in pairs(properties) do
            exports.ox_inventory:RegisterStash('property_' .. v.id .. '_propertystorage', "Coffre", 500, 10000000, false)
        end

        exports.ox_inventory:RegisterStash('adminlocker', "Coffre", 500, 10000000, false)
	end
end)

QBCore.Functions.CreateCallback('ox_inventory:getPlayerNames', function (source, cb, playersList)
    local xPlayer = QBCore.Functions.GetPlayer(source)

    for k,v in pairs(playersList) do
        local xTarget = QBCore.Functions.GetPlayer(v.id)
        if xTarget then
            playersList[k].label = ('[%s] %s'):format(v.id, QBCore.Functions.GetPlayerName(xTarget.PlayerData.source))
        end
    end
    cb(playersList)
end)
...
QBCore.Functions.CreateCallback('MyCity_CoreV2:Ranch:CheckTrunkItems', function (source, cb, data)
    ...
    local result = exports.ox_inventory:GetItem(data.id, 'cow', nil, true)

    if result > 0 then
        exports.ox_inventory:RemoveItem(data.id, 'cow', 1)
        cb(true)
    else
        TriggerClientEvent('QBCore:Notify', xPlayer.PlayerData.source, "Il n'y a pas de vache dans la bétaillère !")
        cb(false)
    end
end)
...
AddEventHandler('esx_inventoryhud_trunk:emptyTrunk', function(data)
    ...
    if not xPlayer or QBCore.Functions.HasPermission(xPlayer.PlayerData.source, 'admin') then
        ...
        exports.ox_inventory:ClearInventory(data.id)
    end
end)

RegisterNetEvent('MyCity_CoreV2:emptyTrunkWipe')
...
local transistepTrolleys = {}
RegisterNetEvent('MyCity_CoreV2:Transistep:StoreTrunkItems')
AddEventHandler('MyCity_CoreV2:Transistep:StoreTrunkItems', function(data)
    local xPlayer = QBCore.Functions.GetPlayer(source)
    if xPlayer.PlayerData.job.name == 'transistep' then
        if not transistepTrolleys[xPlayer.PlayerData.citizenid] then
            inv = Inventory(data.id)
            if not inv then
                local vehicleData = Vehicles['trunk']['models'][data.model] or Vehicles['trunk'][data.class]
                inv = Inventory.Create(data.id, data.plate, 'trunk', vehicleData[1], 0, vehicleData[2], false)
            ...
            transistepTrolleys[xPlayer.PlayerData.citizenid] = {}
            if inv then
                local foundItems
                for k,v in pairs(inv.items) do
                    foundItems = true
                    for l,w in pairs(v) do
                        exports.ox_inventory:RemoveItem(data.id, v.name, v.count)
                    end
                    table.insert(transistepTrolleys[xPlayer.PlayerData.citizenid], {name = v.name, count = v.count, metadata = v.metadata})
                end
                if not foundItems then
                    transistepTrolleys[xPlayer.PlayerData.citizenid] = nil
                    TriggerClientEvent('QBCore:Notify', xPlayer.PlayerData.source, "Le coffre du véhicule est vide")
                end
            end
        else
            TriggerClientEvent('QBCore:Notify', xPlayer.PlayerData.source, "Il y a déja des objets sur votre diable !")
        end
    end
end)

RegisterNetEvent('MyCity_CoreV2:Transistep:GiveTrunkItems')
AddEventHandler('MyCity_CoreV2:Transistep:GiveTrunkItems', function(target)
    local xPlayer = QBCore.Functions.GetPlayer(source)
    if xPlayer and xPlayer.PlayerData.job.name == 'transistep' then
        if transistepTrolleys[xPlayer.PlayerData.citizenid] then
            for k,v in pairs(transistepTrolleys[xPlayer.PlayerData.citizenid]) do
                exports.ox_inventory:AddItem(target, v.name, v.count, v.metadata)
            end
            transistepTrolleys[xPlayer.PlayerData.citizenid] = nil
        else
            TriggerClientEvent('QBCore:Notify', xPlayer.PlayerData.source, "Votre diable est vide")
        end
    end
end)]]
